...
def _summarize_messages(messages: list[dict]) -> str:
    ...
    prompt = (
        ...
        "【输出格式】\n"
        "topics 列表，每个话题包含：\n"
        "- scope: 公开团队事项用 \"team\"，其余一律用 \"noise\"（不会展示，无需填写详情）\n"
        "- title: 话题标题，10 字以内\n"
        "- progress: 当前状态，一句话\n"
        "- details: 事实细节，不超过 3 句\n"
        "- actions: 后续动作列表，每项含 assignee_open_id（必须来自 mentions 或 sender_open_id，不能编造）和 text（只写动作内容）\n\n"
        "【纳入规则（scope=team）】\n"
        "团队任务、会议安排、决议、风险、待定问题；被明确作为团队资料引用的链接。\n\n"
        "【排除规则（scope=noise）】\n"
        "个人自我提醒、寒暄、闲聊；请求机器人执行操作（由系统单独处理）；未被明确引用为团队资料的普通链接分享。\n\n"
        "【合并规则】\n"
        "同一会议或任务的多条消息合并为一个话题。\n"
        "会议的具体时间、地点、参与人已由看板系统记录，actions 中不重复写入。\n\n"
        "【人员规则】\n"
        "消息中的'我'指该条消息的 sender_open_id；无法确定人员时 assignee_open_id 留空，不要编造。\n\n"
        "总长度控制在 500 字以内，不要解释筛选过程。\n\n"
